<script setup>
    ...
    async function deleteCountry() {
        ...
        emit('getCountries')
        emit('closeDialog', `Country ${country.value.name} has been deleted`)
        ...
        <div class="relative p-4 w-full max-w-md h-full md:h-auto">
            ...
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                ...
                <div class="py-6 px-6 lg:px-8">
                    <h3 class="mb-4 text-xl font-medium text-gray-900 dark:text-white">Delete country</h3>
                    <div class="space-y-6">
                        <div>
                            <span>Do you want to delete {{ country.name }}?</span>
                        </div>
                        <button @click="deleteCountry" class="w-full text-white bg-red-700 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Delete</button>
                    </div>
                </div>
            </div>
        </div>
