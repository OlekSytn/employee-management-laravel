<script setup>
    ...
    const emit = defineEmits(['getCities', 'closeDialog'])

    const { dialogVisible, city } = toRefs(props)
    ...
    watch(dialogVisible, async (dialogVisible, prevVal) => {
        ...
        formCity.stateId = city.value.state_id
        formCity.name = city.value.name
    })
    ...
    async function updateCity() {
        ...
        const result = 
            await fetch(`/api/cities/${city.value.id}`, {
        ...
        emit('getCities')
        emit('closeDialog', `City ${formCity.name} has been updated`)

        Object.assign(formCity, form)
        ...
        <div class="relative p-4 w-full max-w-md h-full md:h-auto">
            ...
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                ...
                <div class="py-6 px-6 lg:px-8">
                    ...
                    <div class="width-300">
                        <div>
                            <label for="small" class="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-400">State</label>
                            <select 
                                v-model="formCity.stateId"
                                ...
                                <option v-for="(state, index) in states" :selected="state.id == formCity.stateId" :key="index" :value="state.id"> {{ state.name }} </option>
                            </select>
                        </div>
                        <div class="my-0">
                            <label for="name" class="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-300">City Name</label>
                            <input type="text" v-model="formCity.name" name="name" id="name" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white" placeholder="" required="">
                        </div>
                        <button @click="updateCity" class="mt-4 w-full text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Update</button>
                    </div>
                </div>
            </div>
        </div>
