<script setup>
    ...
    async function getUsers() {
        const result = await fetch(`/api/users?search=${keyword.value}`).then(response => response.json())
        users.value = result
...
<template>
    ...
    <BreezeAuthenticatedLayout>
                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6 bg-white border-b border-gray-200">
                        ...
                        <div class="flex justify-between relative">
                            ...
                            <input @keyup.enter="getUsers" v-model="keyword" type="text" id="email-address-icon" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-1/3 pl-10 p-2.5  dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Search">
                                ...
                                <tbody>
                                    <tr v-for="(user, index) in users" :key="index" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                        ...
                                        <td class="flex px-6 py-4 text-right">
                                            <a @click="showUpdateDialog(user)" class="cursor-pointer">
                                                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                            </a>
                                            <a class="ml-4 cursor-pointer"  @click="showDeleteDialog(user)">
        ...
        <CreateUser :dialogVisible="openCreateDialog" @closeDialog="closeDialog" @getUsers="getUsers" />
        <UpdateUser :dialogVisible="openUpdateDialog" :user="user" @closeDialog="closeDialog" @getUsers="getUsers" />
        <DeleteUser :dialogVisible="openDeleteDialog" :user="user" @closeDialog="closeDialog" @getUsers="getUsers" />
    </BreezeAuthenticatedLayout>
</template>
